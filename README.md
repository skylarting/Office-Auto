# Office-Auto

Windows Excel 单元格批量汇总工具。

## 功能

- 选择一个 `.xlsx` / `.xlsm` 文件；
- 选择文件夹后确认并批量处理其中全部工作簿；
- 支持手动输入或在表格网格中点击选择需要提取的单元格；
- 支持逗号、空格、换行和中英文分号分隔；
- 两种单元格选择方式实时同步；
- 保留真实数值，并复制源单元格数字显示格式；
- 可新建独立汇总文件；
- 可在每个源工作簿最前面插入或替换“汇总”工作表；
- 文件夹生成独立汇总文件时，自动增加“文件名称”和“Sheet名称”列；
- 自动跳过临时文件及已经生成的汇总文件；
- 支持 `.xlsm`，写回源文件时保留 VBA 项目。

## EXE 使用方法

1. 从 GitHub Actions 下载并解压 `ExcelSummary-Windows`；
2. 双击 `ExcelSummary.exe`；
3. 点击“选择表格文件”或“选择文件夹”。选择文件夹后，程序会显示找到的
   Excel 文件数量，并询问是否处理该文件夹下的所有表格；
4. 选择需要提取的单元格，可使用以下任一方式：

   - **手动输入**：填写单元格地址，例如：

   ```text
   I8, J8, L8, I17, J17, L17
   ```

   - **点击选择**：切换到“点击选择”，在带行号、列号的网格中点击单元格。
     再次点击可取消；需要更大范围时可调整显示行数和结束列。网格最多显示
     200 行、AZ 列，更远的地址仍可手动输入；

5. 选择输出方式：

   - **新建表格文件**：界面只显示“输出文件”，单文件生成一个汇总文件；
     文件夹模式将所有工作簿合并到一个汇总文件；
   - **插入表单**：界面只显示“表单名称”，在每个源工作簿最前面插入或
     替换该名称的汇总表单。

6. 点击“开始提取”。“处理进度”区域会显示百分比、当前文件和处理状态；
   每次修改数据来源、单元格或输出设置后，进度会自动重置。

> 写入源工作簿会修改原文件，建议提前备份。

## 公式单元格

程序读取 Excel 文件中上次保存的公式计算结果。如果公式结果为空，请先用
Excel 或 WPS 打开源文件、完成计算并保存，再运行本工具。

## 本地运行

```bash
python -m pip install openpyxl
python export_summary.py
```

命令行模式仍然可用：

```bash
python export_summary.py "测试数据.xlsx" \
  -o "汇总结果.xlsx" \
  -c "I8,J8,L8"
```

## 测试

```bash
python -m unittest -v
```

## 构建 Windows EXE

推送到 GitHub 后，`Build Windows EXE` 工作流会在 Windows 环境中运行测试并
生成 `ExcelSummary.exe`。在 Actions 运行页面底部下载
`ExcelSummary-Windows` 构建产物即可。
