...
def suggest_studio_mappings(
    ...
    for pair in best.values():
        ...
        # Defense in depth for plans generated by older matching engines.
        unique_matches: list[SmartMatch] = []
        used_sources: set[str] = set()
        used_targets: set[str] = set()
        for item in sorted(matches, key=lambda value: (-value.score, value.source_address)):
            if item.source_address in used_sources or item.target_address in used_targets:
                continue
            unique_matches.append(item)
            used_sources.add(item.source_address)
            used_targets.add(item.target_address)
...
class SheetMappingDialog(QDialog):
    def __init__(
        ...
        selectors.addWidget(QLabel("来源工作簿："))
        self.source_book = QComboBox()
        for path in source_files:
            self.source_book.addItem(path.name, str(path.resolve()))
        selectors.addWidget(self.source_book, 1)
        selectors.addWidget(QLabel("来源工作表："))
        self.source_sheet = QComboBox()
        selectors.addWidget(self.source_sheet)
        selectors.addSpacing(18)
        selectors.addWidget(QLabel("目标工作簿："))
        self.target_book = QComboBox()
        for path in target_files:
            self.target_book.addItem(path.name, str(path.resolve()))
        selectors.addWidget(self.target_book, 1)
        selectors.addWidget(QLabel("目标工作表："))
        self.target_sheet = QComboBox()
        selectors.addWidget(self.target_sheet)
    ...
    def refresh_summary(self) -> None:
        ...
        self.source_summary.setText(
            f"来源当前实际选中（{len(source)}）：{format_cell_addresses(source) or '尚未选择'}"
        )
        self.target_summary.setText(
            f"目标当前实际选中（{len(target)}）：{format_cell_addresses(target) or '尚未选择'}"
        )
    ...
    def selector_changed(self) -> None:
        ...
        try:
            ...
            StudioWindow.refresh_suggestion(self.mapping)
            self.viewer.load_pair(
